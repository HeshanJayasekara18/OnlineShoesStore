...
export const Cart: React.FC<CartProps> = ({ isScrolled, variant }) => {
  ...
  if (variant === 'navbar-desktop') {
    return (
      <button 
        ...
        className={`text-white/70 hover:text-white transition-all duration-500 relative group animate-in fade-in zoom-in ${
  ...
  if (variant === 'navbar-mobile') {
    return (
      <button 
        ...
        className={`text-white p-3 backdrop-blur-xl bg-black/40 rounded-2xl border border-white/10 shadow-xl relative group active:scale-95 transition-all duration-500 animate-in fade-in zoom-in ${
        isScrolled ? "opacity-0 scale-50 pointer-events-none" : "opacity-100 scale-100"
      }`}>
  ...
  if (variant === 'floating') {
    return (
      ...
      <button 
        id="cart-icon-floating"
        onClick={toggleCart}
        className="relative group bg-blue-600 hover:bg-blue-500 text-white p-6 rounded-2xl shadow-[0_20px_50px_-15px_rgba(37,99,235,0.6)] transition-all duration-300 hover:scale-110 active:scale-95 border border-white/20"
      >
