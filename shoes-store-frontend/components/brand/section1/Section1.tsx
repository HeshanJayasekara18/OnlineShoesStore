        ...
        <div className="max-w-7xl mx-auto">
          <header className="mb-20">
            <h1 className="text-6xl md:text-8xl font-black tracking-tighter mb-4 animate-in fade-in slide-in-from-bottom-8 duration-700">
              ELITE <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-black to-black">BRANDS</span>
            </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {brands.map((brand, index) => (
              <div 
                key={brand.name}
                className="group relative h-96 rounded-[40px] overflow-hidden border border-white/5 bg-white/5 backdrop-blur-sm hover:border-white/20 transition-all duration-500 hover:scale-[1.02] hover:-translate-y-2 animate-in fade-in slide-in-from-bottom-8"
                style={{ animationDelay: `${(index + 3) * 100}ms` }}
              >
                {/* Background Gradient */}
                <div className={`absolute inset-0 opacity-0 group-hover:opacity-20 transition-opacity duration-500 bg-gradient-to-br ${brand.color} dark:bg-gradient-to-br dark:${brand.color}`} />
                
                {/* Brand Image */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full flex items-center justify-center backdrop-opacity-100 group-hover:opacity-100 transition-all duration-700 group-hover:scale-110 pointer-events-none">
                  <img 
                    src={brand.image} 
                    alt={brand.name} 
                ...
                <div className="relative h-full p-10 flex flex-col justify-between z-10">
                  <div className="bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-white/10 w-fit">
                    <h2 className="text-3xl font-black tracking-tighter mb-2 text-white">{brand.name}</h2>
                    <p className="text-white/40 font-medium group-hover:text-white/70 transition-colors uppercase text-[10px] tracking-widest">{brand.description}</p>
                  </div>
                  
                  <button className="self-start px-8 py-4 rounded-2xl bg-white text-black font-black text-xs tracking-widest hover:bg-cyan-400 hover:text-black transition-all duration-500 opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 shadow-2xl">
                    EXPLORE COLLECTION
                  </button>
                </div>
