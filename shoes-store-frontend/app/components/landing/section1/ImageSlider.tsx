...
export function ImageSlider({
  ...
  return (
    <div className="relative w-full group/slider">
      {/* Slider Container */}
      <div className="relative rounded-[40px] md:rounded-[60px] overflow-hidden shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)] bg-white aspect-4/5 sm:aspect-[1.5/1] md:aspect-video border border-gray-100 transition-all duration-700">
        ...
        {images.map((image, index) => (
          <div
            key={index}
            className={`absolute inset-0 transition-all duration-1000 ease-[cubic-bezier(0.23,1,0.32,1)] p-8 sm:p-16 md:p-24 ${
              index === currentIndex 
        ...
        <div className="absolute top-8 left-8 md:top-12 md:left-12 z-10">
          <div className="backdrop-blur-xl bg-white/80 border border-white/60 p-6 md:p-8 rounded-[32px] shadow-2xl space-y-4 max-w-[280px] md:max-w-md transform transition-all duration-700 delay-300">
            <span className={`text-xs md:text-sm font-bold uppercase tracking-[0.2em] ${currentTheme.text}`}>
              New Collection 2024
            </span>
            <h2 className="text-3xl md:text-5xl font-black text-black leading-tight drop-shadow-sm">
              FEEL THE BURN<br />
            ...
            <div className="flex items-center gap-4 pt-2">
              <button className="bg-black text-white px-8 py-4 rounded-2xl font-bold hover:bg-gray-800 transition-all active:scale-95 text-sm md:text-base shadow-lg shadow-black/20 group/btn flex items-center gap-2">
                SHOP NOW
        ...
        <div className="absolute bottom-8 right-8 md:bottom-12 md:right-12 z-20 flex gap-4">
          <button
            ...
            className="backdrop-blur-xl bg-white/60 hover:bg-white border border-white/40 p-4 md:p-5 rounded-3xl transition-all active:scale-90 shadow-xl group/nav"
            aria-label="Previous slide"
          >
            <svg className="w-6 h-6 text-black group-hover/nav:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
